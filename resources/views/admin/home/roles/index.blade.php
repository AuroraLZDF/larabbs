            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <div class="row  m-b-xs">
                        <div class="col-sm-1">
                            <a href="{{ route('admin.roles.create') }}" class="btn btn-sm btn-primary">添加角色</a>
                        </div>
                    </div>
                            ...
                            <table class="table table-bordered table-hover" style="margin-bottom: auto;">
                                ...
                                @foreach($roles as $key => $role)
                                    <tr>
                                        <td>{{ $role->id or '-' }}</td>
                                        <td>{{ $role->name or '-' }}</td>
                                        <td>{{ $role->guard_name or '-' }}</td>
                                        <td>{{ $role->created_at or '-' }}</td>
                                        <td>{{ $role->updated_at or '-' }}</td>
                                        <td>
                                            <a href="{{ route('admin.roles.edit', [$role->id]) }}" class="btn btn-info" >编辑</a>
